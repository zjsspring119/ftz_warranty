...
import com.ouyeelf.ftz.rsdb.imp.domain.change.NStoreRecordChange;
 ...
 */
public class NStoreRecordBeanDao{

	/** The db. */
	private static DB db = MongoUtil.getDB();

	/** The dc. */
	private static DBCollection dc = db.getCollection("nStoreRecordBean");
	 ...
	 */
	public boolean addResource(NStoreRecordBean nStoreRecordBean) {
		DBObject dbo = NStoreRecordChange.change(nStoreRecordBean);
	 ...
	 * @return true, if successful
	 */
	public boolean addResourceAll(List<NStoreRecordBean> nStoreRecordBean) {
		DBObject[] dbos = new DBObject[nStoreRecordBean.size()];

		for (int i = 0, size = nStoreRecordBean.size(); i < size; i++) {
			
			dbos[i] = NStoreRecordChange.change(nStoreRecordBean.get(i));
		}
	 ...
	 * @return true, if successful
	 */
	public boolean removeResourcePhysics(NStoreRecordBean nStoreRecordBean) {
		DBObject dbo = new BasicDBObject();
		dbo.put("storeNo", nStoreRecordBean.getStoreNo());
	 ...
	 * @return true, if successful
	 */
	public boolean updateResource(NStoreRecordBean nStoreRecordBean) {
		if (removeResourcePhysics(nStoreRecordBean)) {
			return addResource(nStoreRecordBean);
		}
